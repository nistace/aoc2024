...
public class Day06 : AbstractDay
{
	...
	public override string Part1()
	{
		...
		Debug(obstacles, startPosition, dimensions, simulated.visited);

		return $"{Simulate(obstacles, startPosition, dimensions).visited.Count}";
	}
	...
	public override string Part2()
	{
		...
		var possibleObstacles = defaultVisited.visited.SelectMany(t => t.Value.Select(u => u.MoveFrom(t.Key))).Distinct().Except(obstacles).Except([startPosition]);

		return $"{possibleObstacles.Count(newObstacle => Simulate(obstacles.Union([newObstacle]).ToHashSet(), startPosition, dimensions).loop)}";
	}

	private static (bool loop, Dictionary<(int x, int y), HashSet<Heading>> visited) Simulate(HashSet<(int x, int y)> obstacles,
	...
	private static void Debug(HashSet<(int x, int y)> obstacles,
		(int x, int y) startPosition,
		(int width, int height) dimensions,
		Dictionary<(int x, int y), HashSet<Heading>> visited)
	{
	...
	{
		var allCharsCoords = GetInputLines().Select((chars, y) => (chars, y)).SelectMany(line => line.chars.Select((c, x) => (c, coords: (x, line.y))))
			.ToDictionary(t => t.coords, t => t.c);
